...
export default (dsProps): DataSetProps => {
  ...
  return {
    ...
    fields: [
      ...
      {
        name: 'customerSystem',
        label: intl.get('hmdm.applyTenant.view.customerSystem').d('对接客户系统'),
        type: FieldType.string,
        required: requiredBylinksType(linksType),
      },
      {
        name: 'createItem',
        label: intl.get('hmdm.applyTenant.view.createItem').d('已创建项目'),
        type: FieldType.string,
      },
      {
      ...
      {
        name: 'deliveryName',
        label: intl.get('hmdm.applyTenant.view.deliveryName').d('汉得交付联系人'),
        type: FieldType.string,
        labelWidth: '110',
      },
      {
        name: 'collection',
        label: intl.get('hmdm.applyTenant.view.collection').d('是否已收款'),
        type: FieldType.string,
        required: requiredBylinksType(linksType),
      },
    ],
