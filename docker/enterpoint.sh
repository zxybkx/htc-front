...
find /usr/share/nginx/html -name '*.js' | xargs sed -i "s BUILD_PLABUILD_API_HOSTTFORM_VERSION $BUILD_PLATFORM_VERSION g"
...
find /usr/share/nginx/html -name '*.js' | xargs sed -i "s GOTO_INVOICE_CHECK_ROUTE $GOTO_INVOICE_CHECK_ROUTE g"
find /usr/share/nginx/html -name '*.html' | xargs sed -i "s GOTO_INVOICE_CHECK_ROUTE $GOTO_INVOICE_CHECK_ROUTE g"
find /usr/share/nginx/html -name '*.css' | xargs sed -i "s GOTO_INVOICE_CHECK_ROUTE $GOTO_INVOICE_CHECK_ROUTE g"
find /usr/share/nginx/html -name '*.js' | xargs sed -i "s INVOICE_CHECK_API_SERVER $INVOICE_CHECK_API_SERVER g"
find /usr/share/nginx/html -name '*.html' | xargs sed -i "s INVOICE_CHECK_API_SERVER $INVOICE_CHECK_API_SERVER g"
find /usr/share/nginx/html -name '*.css' | xargs sed -i "s INVOICE_CHECK_API_SERVER $INVOICE_CHECK_API_SERVER g"
